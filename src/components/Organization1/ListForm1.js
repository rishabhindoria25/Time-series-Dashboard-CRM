...
import ListformEditableRow1 from './ListformEditableRow1';
import Listformtable1 from './Listformtable1';
...
console.log(timestamp)
 const newEditvalueOrg={
  ...
  Rej_res:editFormData.Rej_res,
}

let newEditvalue=[
  editFormData.Time,
  editFormData.Rej_res,
]
...
seteditOrgid(null)
const newvalue= 'NEWTAB Rej_res=\"'+(newEditvalueOrg.Rej_res)+'\" '+timestamp
console.log("newvalue")
console.log(newvalue)
fetch('http://localhost:8080/Rejvalueedit', {
    method: 'POST',
    headers: {
...
const newvalueOrg= 'NEWTAB Rej_res=\"'+(AddvalueOrg.Rej_res)+'\" '+String(Date.now())+'000000'

const newvalueOrgs=[...valueOrg,newvalueOrgnotshow];
setvalueOrg(newvalueOrgs)
fetch('http://localhost:8080/Rejvalueadd', {
    method: 'POST',
    headers: {
      'Access-Control-Allow-Origin': '*',
      'Accept': 'application/json',
                  ...
                  <div className="card-header">
                   ...
                   {/* <form className="data"/> */}
                    ...
                    <table className="data-table">
                     <thead>
                     <tr>
                          <th>Reasons for Rejection/Qty(nos)</th>
                          <th>Action</th>
                       </tr>
                        {/* </tbody> */}
                    </thead>
                    {ShowForm &&(
                        ...
                        <tr>                
                       <td>
                      <input type="text" name="Rej_res"style={{backgroundColor:'#E8F0FE'}} 
                          // value={Oid} 
                         className="form-control" placeholder="Reasons for Rejection"required="text" 
                     ...
                     <button type="submit" 
                       ...
                       */}
                        ...
                        valueOrg && valueOrg.map(article=>(
                          <Fragment>
                            {editOrgid===article[0]?<ListformEditableRow1 editFormData={editFormData} handleCancel={handleCancel} handleEditFormChange={handleEditFormChange}/>:<Listformtable1 article={article} handleDelete={handleDelete} handleEditclick={handleEditclick}/>}
                            {dialog.isLoading && (
                              <Dialog
                                //Update
